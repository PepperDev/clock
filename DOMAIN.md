...
- **Single TLS flag for signal handler**: the SIGINT/SIGTERM handler writes
  ...
  **`tls_terminated` is main-thread-only**: because it is `__thread`, each
  ...
  See [Flow / Lifecycle](#flow--lifecycle) cleanup for full details.
...
4. CLI argument selects display mode: `auto` (default), `text`, `ascii`, or `sixel`.
 5. `auto` mode re-evaluates at startup: if stdout is a TTY, probe sixel
    support via DA1 query `\033[c` (10ms timeout) — but only when stdin is
    also a TTY, because DA1 sends the query to stdout and reads the
    response from stdin. If sixel-capable → sixel mode; otherwise → ascii
    mode. If stdout is a TTY but stdin is not → ascii mode (DA1 cannot
    complete without stdin). If stdout is not a TTY → text mode.
...
14. HTTP requests (WAN IP, weather) use a three-tier async architecture with parallel DNS threads, a single I/O thread, and a main loop that never touches sockets. Pending results are omitted (WAN) or shown as dash (weather) in continuous mode; `--once` uses a 3-second deadline. Per [Network — Architecture](#architecture).
15. Failed requests enter exponential backoff (4s, 8s, 16s); after 3 failures per stage the target is abandoned until a network-change event resets counters. Per [Network — Retry landscape](#retry-landscape).
16. No kernel module loading — hwmon drivers must already be loaded for sensor access.
17. Netlink event monitoring uses a dedicated thread with a mutex-protected FIFO drained each tick. Per [Network](#network) and [Monitor Thread Action FIFO](#monitor-thread-action-fifo).
...
Without container: `<icon|MEM> NN% N.N/N.NM / N.N/N.NG`. With container:
`<icon|MEM> NN% N.N/N.NG NN% N.N/N.NG` — first percentage/value pair is
container (guest), second is host. Each pair follows unit selection
independently per [Data-size thresholds](#data-size-thresholds), using its
own max value for the threshold decision. All values stored in **kB**
internally; display divides by 1024.0. Container values from cgroup v2/v1;
when `memory.max` is `"max"` (v2 unlimited) or `memory.limit_in_bytes`
is `-1` (v1 unlimited), `ctr_max_kb` comes from `MemTotal` (already in kB)
...
Padding between throughput and bars aligns bars to a fixed column: the
throughput strings (rx + tx) occupy up to 11 characters, with 1–7 spaces
of padding filling the gap. Text mode uses a single space. This keeps the
bars and dBm value at a static horizontal position regardless of
throughput string width.
...
| `MEM` | `MEM` | 🧠 (`U+1F9E0`) | Per [Memory display format](#memory-display-format). | Percentage per [Format Rules](#percentage-display). Unit per [Data-size thresholds](#data-size-thresholds). Container values from cgroup v2/v1 — per [Container memory](#machine-monitoring--data-sources). |
| `FAN` | `FAN` | 💨 (`U+1F4A8`) | Per [FAN widget output order](#hardware-sensor-discovery-order). | Fans space-separated on one line, temps on next line. Newline (not blank line) between groups. Output order per [sensor discovery](#hardware-sensor-discovery-order). |
| `BAT` | `BAT` | *(dynamic)* | Text mode: `BAT NN% charging/discharging/full`; Ascii/sixel: `<icon:⚡🔋🪫> NN%` (icon before percentage, no `BAT` label). Icons per [Battery icons](#battery-icons). | After percentage, append space + arrow + time estimate when samples exist: `↓ 2h 15m` (discharging) or `↑ 45m` (charging). When no samples exist, no arrow/time appended. Full/Not charging always shows no estimate. Ascii/sixel mode: same arrow/time appended. |
| `UP` | `UP` | ⏱️ (`U+23F1` + VS16) | `<text:UP / icon:⏱️> Nd Nh Nm` |
| `STO` | `STO` | 🗄️ (`U+1F5C4` + VS16) | Per [Storage widget rendering](#storage-widget-rendering). | Throughput unit per [Data-size thresholds](#data-size-thresholds) (b→K→M chain). Usage unit per same rules. Each direction (read/write) has independent unit. Arrow: ↓ read, ↑ write. Mount prefix: ascii/sixel mode only (📁 `U+1F4C1`); text mode has no mount prefix. One space between icon/prefix and mount point. |
| `NET` | *(none)* | 🌐 (`U+1F310`) | Each NIC on its own line (no blank lines). Ascii/sixel mode: every NIC line is prefixed with an icon — 🌐 for wired, 📶 (`U+1F4F6`) for wireless — followed by a space, then `iface ↓N↑N/s [linkspeed]`; wireless when present: `wlan0 ↓N↑N/s ▂▄▆█ -NNdBm` (bars and padding per [Wireless signal bars](#wireless-signal-bars)); SSID line (ascii/sixel): `<icon:🛜> <name> N.N/N.NG` / `<icon:🛜> <name> NN/NNM` — text mode: `SSID <name> ...`; then address lines in fixed order: IPv4, IPv6 (local), WAN (public), WAN6 (public). Each line is shown only when its address is non-empty. Text mode labels: `IP`/`IP6`/`WAN`/`WAN6`; ascii/sixel icons: 📡/📡/🌍/🌍. When a local IP matches its WAN counterpart, its label/icon changes and the redundant WAN/WAN6 line is omitted, per [Overlapping IP/WAN](#mode-specific-rendering). | Throughput per [Network throughput thresholds](#network-throughput-thresholds). Wireless rate per [Wireless link rate](#wireless-link-rate). Link speed per [Wired link speed](#wired-link-speed). Arrow: ↓ rx, ↑ tx. |
...
For widget-specific content (icons, labels, format strings) per mode, see [Widget rendering details](#widget-rendering-details) and [Battery icons](#battery-icons). The table below covers cross-cutting concerns that span multiple widgets.
...
| **CPU freq / governor** | Discovery phase: `glob("/sys/devices/system/cpu/cpu*/cpufreq")` to find policy dirs; `strdup` each into `keep.cpu_freq_dirs[]`; `sys_access` cpu0's `scaling_governor` for existence (do not read value). Per tick: iterate cached policy dirs for `scaling_cur_freq`, sum and average; same for `scaling_max_freq`. Governor from cpu0 only (all share the same policy). | CPU widget active |
...
| **Container memory** | Discovery phase: `sys_access("/sys/fs/cgroup/memory.current", F_OK)` (v2) with fallback `sys_access("/sys/fs/cgroup/memory/memory.usage_in_bytes", F_OK)` (v1). Cache cgroup version in `keep.mem_flags`. Per tick: read the detected cgroup files for usage and limit. Raw cgroup values are in **bytes**; divide by 1024 to store in **kB** (`ctr_used_kb`, `ctr_max_kb`). When `memory.current` exists but `memory.max` is `"max"` (v2 unlimited), set `ctr_max_kb` directly from `MemTotal` (already parsed from `/proc/meminfo` in kB — no multiply/divide needed). When `memory.limit_in_bytes` is `-1` (v1 unlimited — the kernel writes `(unsigned long)-1`, so the value is the maximum unsigned representation), treat the same way. Display formatting divides by 1024.0 to convert kB → MB/GB per [Data-size thresholds](#data-size-thresholds).
| **Battery** | Discovery phase: `glob("/sys/class/power_supply/*/type")`, pick first with value `Battery`. Cache the prefix path. If none found, `bat_pct` stays -1 and BAT line is omitted. Detect `charge_now` vs `energy_now` type. Read `charge_full` (fallback `energy_full`) once. Per tick: use the cached prefix + type flags, no re-discovery. Read `charge_now` (fallback `energy_now`) for raw µAh value. Re-read `charge_full` only when state transitions to Full/Not-charging and `bat_pct != 100`. Compute display percentage as `(charge_now * 100) / charge_full`. Read `status`. Icons per [Battery icons](#battery-icons). On each tick, record raw charge and wall-clock timestamp (`time(0)`) into FIFO samples per [Battery time estimation](#battery-time-estimation). | BAT widget active |
| **Network throughput** | rtnetlink `RTM_GETLINK` — first per-tick call (or route-change/link event) does a full dump of ALL interfaces populating the **link cache** (`r->links[]`). Steady-state ticks send targeted `RTM_GETLINK` queries for only the SELECTED interfaces (at most 3). `rtnl_read_dev()` reads cached `rx_bytes`/`tx_bytes` from `IFLA_STATS64` and computes deltas against the previous tick's raw values. | NET widget active |
...
| **Wireless signal** | netlink nl80211 `NL80211_CMD_GET_STATION` — targeted query with `NL80211_ATTR_IFINDEX` + `NL80211_ATTR_MAC` (no `NLM_F_DUMP`). BSS MAC cached in `net_ctx.bss_mac`, refreshed via `GET_SCAN` (dump with IFINDEX) on local IP change or `GET_STATION` failure. Signal strength in dBm. Only for the selected wlan NIC (at most one, placed last on its own line). | NET widget active |
| **Wireless link speed** | netlink nl80211 — `NL80211_CMD_GET_STATION` targeted query (same as signal) for tx_bitrate and rx_bitrate. Queried only for the selected wlan NIC. Requires a persistent `NETLINK_GENERIC` socket held across intervals. Format per [Format Rules — Wireless link rate](#wireless-link-rate). | NET widget active |
| **Wireless SSID** | netlink nl80211 — `NL80211_CMD_GET_SCAN` (dump with `NL80211_ATTR_IFINDEX`) returns BSS entries; SSID is extracted from the BSS Information Elements. Cached in `net_ctx.wlan_ssid`; refreshed on local IP change and on `GET_STATION` failure (which also refreshes the BSS MAC). Fallback: interface name. | NET widget active |
...
| **CPU temperature** | Discovery phase: `glob(k10temp)` with fallback `glob(coretemp.0)` to discover hwmon path; cache in `keep.cpu_temp_path`. Per tick: read from cached path. | CPU widget active |
| **Storage temperature** | Discovery per device (lazy, when a block device is first encountered in per-tick enumeration): `/sys/block/nvme*n1/device/hwmon*/temp*_input` (NVMe only). Probe via `glob()` — the path contains a wildcard suffix (`hwmon*/temp*_input`) so `access()` on the pattern alone would fail. Cache the resolved path on success; mark absent and skip further probing on failure. NVMe hwmon path may be absent on some controllers. SCSI/SATA storage temperature is not supported (no hwmon). | STO widget active |
| **GPU usage** | `/sys/class/drm/card*/device/gpu_busy_percent` or `/sys/class/drm/card*/power/rc6_residency_ms` (delta). Hot-pluggable eGPUs not supported — probe once at startup, never re-probe (discovery phase). Cache per-file presence flags (`gpu.present` bits) — files not found during discovery are never retried. Per tick: read only files confirmed present. If absent, GPU usage is omitted. | GPU widget active |
| **GPU core clock** | `/sys/class/drm/card*/device/pp_dpm_sclk` (active + max from `*` marker / last line). Hot-pluggable eGPUs not supported — probe once at startup, never re-probe (discovery phase). `access()` to detect AMD vs Intel flow (`gt_act_freq_mhz`/`gt_max_freq_mhz`). Cache per-file presence flags — files not found are never retried. Per tick: read only confirmed present files. If both absent, clock line omitted. | GPU widget active |
| **GPU memory clock** | `/sys/class/drm/card*/device/pp_dpm_mclk` (active + max from `*` marker / last line). Hot-pluggable eGPUs not supported — probe once at startup, never re-probe (discovery phase). `access()` to check existence. Cache presence flag — never retried on failure. Per tick: read only if confirmed present. If absent, memory clock omitted. | GPU widget active |
...
| **GPU temperature** | `/sys/class/drm/card*/device/hwmon/hwmon*/temp1_input`. Hot-pluggable eGPUs not supported — probe once at startup, never re-probe (discovery phase). Discover path via `access()` or `glob()`. Cache path + presence flag — never retried on failure. Per tick: read only if confirmed present. | GPU widget active |
| **GPU fan** | `/sys/class/drm/card*/device/hwmon/hwmon*/fan1_input`. Hot-pluggable eGPUs not supported — probe once at startup, never re-probe (discovery phase). Discover via `access()` or `glob()`. Cache path + presence flag — never retried on failure. Per tick: read only if confirmed present. | GPU widget active |
| **GPU governor** | `/sys/class/drm/card*/device/power_dpm_force_performance_level`. Hot-pluggable eGPUs not supported — probe once at startup, never re-probe (discovery phase). `access()` to check existence. Cache presence flag — never retried on failure. Per tick: read only if confirmed present. | GPU widget active |
| **Motherboard fans** | Discovery phase: locate the hwmon directory via the discovery glob, then enumerate `*_input` files within it and cache the full file paths. Per tick: only `read_uint()` on each cached path — no `glob()`, no `malloc()`/`free()` of the entry array, no filesystem re-enumeration. Sorted before temperatures — all fan entries emitted first on the fan line, then temperatures on a separate line. | FAN widget active |
| **Motherboard temperatures** | Scanned in the same `*_input` glob pass as fans in the discovery phase; identical per-path caching. Per tick: only `read_uint()` on each cached path. Emitted after all fan entries, separated by a newline when both groups are present. | FAN widget active |

### Hardware sensor discovery order

Within the initialization phase, hardware sensors are probed in this order,
guarded by the active widget set:
...
HTTP requests (WAN IP, weather) use a **three-tier async architecture**:
...
While a response is pending in continuous mode: WAN/WAN6 lines are
...
Netlink event monitoring uses a **monitor thread** holding a dedicated
...
If all 10 fail, skip this tick. The main loop aggregates events and
applies the aggregate per
[the processing rules](#main-thread-processing-per-tick).
...
Network-specific sources below. For the full list (system stats, GPU, battery, sensors), see [Machine Monitoring — Data Sources](#machine-monitoring--data-sources).

| Source | Method | Detail |
|--------|--------|--------|
| Public IPv4/IPv6 | HTTP GET via async I/O thread | `api.ipify.org` / `api6.ipify.org` \u2014 per [Data Sources](#machine-monitoring--data-sources) |
| Local IPv4 | `SIOCGIFADDR` ioctl | Per selected primary NIC |
| Local IPv6 | rtnetlink `RTM_GETADDR` (`rtnl_find_addr6`) | Per selected primary NIC |
| Default routes | rtnetlink `RTM_GETROUTE` | `rtnl_default_v4`, `rtnl_default_v6` \u2014 main table only |
| Throughput | rtnetlink `RTM_GETLINK` | Link cache; targeted queries for selected NICs |
| Wireless signal | nl80211 `NL80211_CMD_GET_STATION` | Targeted (IFINDEX + cached MAC from `GET_SCAN`), selected wlan NIC only |
| Wireless rate | nl80211 `NL80211_CMD_GET_STATION` | Targeted (IFINDEX + cached MAC from `GET_SCAN`), tx/rx bitrate for selected wlan NIC |
| Wireless SSID | nl80211 `NL80211_CMD_GET_SCAN` | Dump (IFINDEX); cached, refreshed on local IP change and GET_STATION failure |
| Link speed (wired) | `ETHTOOL_GLINKSETTINGS` ioctl | Lazy, cached in link cache entry |
| Weather | HTTP GET via async I/O thread | `wttr.in/?format=j1` |
...
- `--once` mode: single attempt per fetch with a hard **3-second total
   deadline**. Uses the same cancellation mechanism (per [Flow / Lifecycle](#flow--lifecycle)
   step 3e) to release resources when the deadline expires or all results
   arrive \u2014 no new request follows. Missing results omitted. No retry.
...
If a request fails (timeout, connection refused, parse error), the
previous cycle's data is retained in continuous mode. The failed target
enters exponential backoff (see formulas above). When all retries are
exhausted: the target is abandoned \u2014 the last successfully cached value
is displayed (WAN shows cached IP, weather shows dash if never valid or
cached data if available). Only a network-change event (address add/del
or link add/del from the monitor FIFO) resets the try counters and
...
After `poll_refresh` and `pick_primary`, `addr4_changed`/`addr6_changed` are
...
### Public IP overlap detection

WAN (public IPv4) is compared against
local IPv4. WAN6 (public IPv6) is compared against local IPv6. If a
public IP matches its local counterpart, the local IP line's label changes
to indicate the overlap: in text mode, `IP` becomes `WIP` and `IP6` becomes
`WIP6`; in ascii/sixel mode, the 📡 icon is replaced by 🌍. When they
match, the redundant WAN/WAN6 line is omitted — the IP line already bears
the WAN address. Line order is always: IPv4, IPv6, WAN, WAN6 (each
omitted when empty). IPv4 and IPv6 comparisons are independent; a
mismatch on one family does not affect the other.

### Link cache

Per-tick storage for interface metadata and throughput counters (`r->links[]`).
Each entry:
...
[Monitor Thread Action FIFO](#monitor-thread-action-fifo))
...
via a targeted nl80211 `NL80211_CMD_GET_STATION` query (using
`NL80211_ATTR_IFINDEX` + `NL80211_ATTR_MAC`, no `NLM_F_DUMP`) \u2014 not part of the
link cache. The BSS MAC is cached in `net_ctx.bss_mac` and refreshed
via `NL80211_CMD_GET_SCAN` (dump with IFINDEX) on local IP change or `GET_STATION` failure.
The same `GET_SCAN` call also refreshes the cached SSID. No
`GET_STATION` dump is performed.
...
A per-tick
bidirectional lookup table (`name_idx`) that eliminates redundant
...
Use rtnetlink `RTM_GETROUTE` dump
...
### Primary NIC selection

At most three interfaces may be shown, with
deduplication:
...
Each NIC appears on its own line. No blank lines between NICs.
The wired lines loop (skipping the wireless index) outputs up to 2
wired NICs. The wireless line output adds 1 more. Total output is
at most 3 NIC lines.

### Virtual NIC filtering

After `gather_routes` fills `gw_idx[]`
...
### Wireless NIC selection

The RTM link dump alone cannot identify
...
client-mode wireless NICs in one call. The dump result caches only
the ifindex of each wireless interface (no SSID \u2014 SSID is cached
separately via `GET_SCAN`). The dump is cached with a `fresh` flag.
...
- **Initial / NIC attach/detach**: perform a full nl80211 dump
  ...
  (highest-priority) route \u2014 this is the selected wireless NIC. Its
   ifindex is stored. A `GET_SCAN` query (dump with IFINDEX) is issued immediately to
  populate the SSID + BSS MAC cache (see below).
- **Route change only** (no NIC add/remove): the cached dump is still
  ...
  dump to pick the wireless NIC.
- **Local IP change**: a fresh SSID + BSS MAC are needed (the NIC may
  have reconnected to a different AP). Perform a targeted
   `NL80211_CMD_GET_SCAN` query (dump with IFINDEX) on the selected wireless NIC to
  update both the cached SSID and BSS MAC.

After the initial selection, `fresh` is set to `false`. A fresh dump
only occurs again when interfaces are added or removed.

**Wireless SSID + BSS MAC cache**: the SSID and BSS MAC are cached
in `net_ctx` (fields `wlan_ssid` and `bss_mac`). They are refreshed
on:

1. Initial NIC selection (after `GET_INTERFACE` dump).
2. Local IP change (same trigger as the old `GET_INTERFACE` targeted
   SSID refresh, but now `GET_SCAN` provides both SSID and BSS MAC
   in one call).
3. `GET_STATION` failure (see below) \u2014 a stale BSS MAC causes
   `GET_STATION` to return an error, triggering a `GET_SCAN` refresh
   and retry.

**Per-tick wireless queries**: every tick, for the selected wireless
NIC (at most one):

1. `NL80211_CMD_GET_STATION` (targeted, `NLM_F_REQUEST` only) with
   `NL80211_ATTR_IFINDEX` + `NL80211_ATTR_MAC` set to the cached
   BSS MAC \u2014 returns signal strength (dBm) and tx/rx bitrate.
   No `NLM_F_DUMP` is used; no `GET_STATION` dump is ever performed.
2. **If step 1 fails** (stale BSS MAC from roaming without IP change):
    call `NL80211_CMD_GET_SCAN` (dump with IFINDEX) to refresh the SSID + BSS
   MAC cache, then retry `GET_STATION` with the updated BSS MAC.
...
Each tick filters virtual filesystems via a dynamic skip list populated
once at startup. The skip list is seeded with the 28 previously hardcoded
entries, then extended by reading `/proc/filesystems` and
collecting every `nodev` entry (deduplicated). After
population, the list is shrink-to-fit (immutable thereafter). Matching
uses prefix comparison on the filesystem name. Mounts with
major number 7 (loop devices) are also filtered. The remaining mounts
are re-matched to block devices each tick.
...
regardless of hwmon name or path prefix. Probe via `glob()` — the path
contains a wildcard suffix so `access()` on the pattern alone would fail.
Cache the resolved path on success; mark absent and skip further probing
on failure. Shown as `NN°C` per [Format Rules](#temperature-display) before device size on
...
appear before any device line, as a group of standalone mount lines —
prefixed with the icon in ascii/sixel mode (same as matched mounts) or
unprefixed in text mode. Multiple devices are separated by line breaks (not blank
...
Example layout:
```
📁 /mnt/crypt 65% 200/300G           ← unmatched mount, ascii/sixel prefix
...
## Flow / Lifecycle
 ...
 2. **Terminal setup** (TTY only, not `--once` or text mode) —
    ...
    a. If `*mode == MODE_AUTO`: detect sixel capability via DA1 query
       (`\033[c`) and resolve to sixel or ascii mode
       (see [Sixel Mode Design — Sixel capability detection](#sixel-capability-detection)
       for full details).
  ...
  3. **Initialization and discovery phase** (single-shot, called from `clock_main`
     after `setup_tty_and_mode`, before `clock_loop`, NOT per tick):
      For each active widget, probe the system once and cache all discovery
      results in `struct cpu_keep`. Per-tick data collection later reads from
      these cached paths — no re-discovery, no flag checks for "first time".
      Discoveries guarded by the active widget set (if a widget is inactive,
      its hardware is never probed).

      **Existence vs value**: discovery uses `sys_access(F_OK)` when only a
      file's presence must be confirmed — the actual value will be read in the
      per-tick path. `sys_glob` is used when a path pattern must be resolved to
      a concrete path. `open` + `read` is used only when the file content itself
      is the discovery (e.g. reading the `type` file of a power supply to know
      if it's `Battery`). A file found by `access` or `glob` during discovery is
      cached by path; the per-tick path reads from the cached path without
      re-probing.

      a. **CPU** (CPU widget): discover k10temp/coretemp hwmon path via
         `sys_glob`; discover cpufreq policy dirs via
         `sys_glob("/sys/devices/system/cpu/cpu*/cpufreq")`; read CPU count
         from `/proc/stat` line count. Cache governor path existence via
         `sys_access` on cpu0's `scaling_governor` — do not read the value,
         per-tick reads the actual governor string. Store all in
         `keep.cpu_temp_path`, `keep.cpu_freq_dirs[]`, `keep.cpu_freq_ndir`.

      b. **Battery** (BAT widget): discover power supply directory via
         `sys_glob("/sys/class/power_supply/*/type")`, pick first with value
         `Battery`. Detect whether `charge_now` or `energy_now` exists via
         `sys_access` (not by attempting to open+read). Detect whether
         `charge_full` or `energy_full` suffix exists via `sys_access`. Store
         prefix path and type flags in `keep.bat_prefix`, `keep.bat_flags`.
         Read the initial `charge_full` (fallback `energy_full`) value via a
         shared helper `bat_update_full()` — same function used by
         recalibration. On the initial call the read is expected to succeed;
         on recalibration the same helper is called but silently keeps the
         old value if the read fails.

      c. **GPU** (GPU widget): discover DRM card directory via
         `sys_glob("/sys/class/drm/card[0-9]*")`; detect AMD via
         `sys_access(gpu_busy_percent)`, Intel via
         `sys_access(gt_act_freq_mhz)` + `sys_access(gt_max_freq_mhz)`;
         discover hwmon dir for temperature and fan via `sys_glob`; probe
         feature files via `sys_access` (`pp_dpm_sclk`, `pp_dpm_mclk`,
         `mem_info_vis_vram_total`, `power_dpm_force_performance_level`,
         `rc6_residency_ms`). Store in `gpu.card_path`, `gpu.hwmon_path`,
         `gpu.present` bits. No files are read during discovery — all
         actual values are read per tick from the cached paths.

      d. **Motherboard fans/temps** (FAN widget): discover hwmon directory via
         prioritized probe (Dell → ASUS → NCT6687 → fallback `/sys/class/hwmon/*`
         with driver-name filtering). Enumerate `*_input` entries. Store in
         `keep.mobo_hwmon`, `keep.fan_ents[]`, `keep.fan_n`,
         `keep.fan_cache_dir`, `keep.fan_fb_valid`.

      e. **Container memory** (MEM widget): detect cgroup v2 vs v1 via
         `sys_access`. Store flags in `keep.mem_flags`.

  4. **Main loop** (unconditional single pass if `--once`):
     ...
     b. Convert to broken-down time via `localtime_r` (reentrant, thread-safe).
      c. For each active widget, collect its metrics — reads from paths
         discovered in step 3, no re-discovery or "first time" probes.
         Skip widgets not in the active set (their paths were never discovered).
        d. Drain the monitor action FIFO per [Network — Architecture](#architecture).
           If all 10 attempts fail, skip this tick.
          ...
          add/del actions), refresh local IPs per
          [Network — Network change handling](#network-change-handling).
      ...
      e. Per-target lifecycle. Each target (wan4, wan6, weather) has
         ...
         Retry mechanism: per [Network — Retry landscape](#retry-landscape).
         On network-change event-driven restart, bypasses all backoff timers.
         ...
         Re-fetch public IP at `--ip-refresh` interval and weather at
          ...
          Main loop does no socket I/O — it reads `struct http_result`
           slots for completed fetch data. Per [Thread / Worker / Parallelism Landscape](#thread--worker--parallelism-landscape).
...
- For I/O, use poll when monitoring a small number of file descriptors (under ~100); prefer epoll for heavy workloads with hundreds or more concurrent connections. Use non-blocking operations, batch processing, lock-free queues, and smart concurrency design.
...
1. **Netlink monitor thread** (`rtnl_mon_thread`): runs `poll(100)` on a
   ...
   and appends typed actions to a mutex-protected FIFO. The thread
   performs no conversion or decision-making beyond type detection and raw
   field extraction. Per [Network — Architecture](#architecture) and
   [Monitor Thread Action FIFO](#monitor-thread-action-fifo).
   ...
   (`m->fd = -1`). The thread's `poll(100)` timeout expires, detects the
    condition and exits → `pthread_join`. Per [Termination](#termination).
...
2. **DNS resolver threads** (up to 3, one per target): each runs blocking
   ...
   I/O thread via `eventfd` (or creates it if none exists). Created joinable
   by main loop in `start_dns`. The thread never self-detaches — detach
   ownership is single-caller per path: I/O thread detaches on success
   (after consuming the result), main thread detaches on cancellation.
   Full lifecycle per the pseudocode diagram below.
...
4. **Main loop** (continuous mode): never blocks on I/O. Each tick:
   drains the monitor FIFO (trylock loop, per [Network — Architecture](#architecture)),
   aggregates events per [the processing rules](#main-thread-processing-per-tick),
   reads `struct http_result` slots, handles retry/backoff per
   [Network — Retry landscape](#retry-landscape), and handles network-change
   restarts per [Network — Network change handling](#network-change-handling).
   Full per-tick flow per [Flow / Lifecycle step 4](#flow--lifecycle).
...
dns_cancel(slot):                    dns_thread_run(arg):
  ...
  pthread_cancel(t)                       unlock; free(arg); return   // NO self-detach — main thread owns detach
  pthread_detach(t)                    else:
  // done — main thread is the single                      slot->addr = ai
  // detach owner on the cancel path                       slot->state = 2
                                        unlock(slot->lock)
                                       ...
                                       // ...
                                        ...
                                        free(arg)
                                        return NULL  // NO self-detach — I/O thread owns detach
```

> **Detach ownership rule**: exactly one caller performs `pthread_detach`
> per thread lifetime. On success the I/O thread detaches (after consuming
> `conn.dns.thread`). On cancellation the main thread detaches (after
> `pthread_cancel`). The DNS thread itself never calls `pthread_detach` —
> this eliminates the double-detach race.
